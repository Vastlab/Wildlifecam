...
*/
  ...
  MetaRecognition( int scores_to_drop=0,   //!< is this object for prediction, if so  how many top scores to drop when fitting
                   ...
                   int translate_amount=100 //!< shifting data to ensure all is positive.. if very broad and you want some probabilities or all you can make it larger.. 
                   );
	...
	int ReNormalize(double *invec, double *outvec, int length);     //!< W-score Renormalize the vecotor invec[0:length-1] into outvec (in and out can be same) return is 1 for success, <0 for error code
        ...
        /// Use FitHight if your data is such that is larger is better.  The code will still transform, and keep parmeters to keep small data away from zero.  If you get scores that are complain about it being negatie, make a MR object with different translate amount
        /// returns 1 for success, <0 for error code
	int FitHigh(double* inputData, int inputDataSize); 
        ...
        /// returns 1 for success, <0 for error code
	int FitLow(double* inputData, int inputDataSize);// 
        ...
        int FitSVM(svm_node_libsvm* SVMdata, int inputDataSize, int label_of_interest =1, bool label_has_positive_score=true, bool reject_complement=1 ); 
