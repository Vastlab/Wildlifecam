                     ...
                     double* censoring, double* frequency, int size)
{
    ...
    memset(expz, 0, sizeof(double)*size);
    memset(L, 0, sizeof(double)*size);

    if (sigma <= 0) {WEIBULL_ERROR_HANDLER(-1,"Bad sigma (shape) in weibull_neg_log_likelihood.."); 
        printf("Bad sigma (shape) in weibull_neg_log_likelihood...\n");
        goto freememory2;}
...
static int wnorminv(double* x, double* p,double *mu, double* sigma, int size)
{
    ...
    double tempP[4], tempMU[4], tempSigma[4], x0[4];
...
int weibull_fit(double* weibullparms, double* wparm_confidenceintervals, double* inputData, double alpha, int size)
{
    ...
    /*  ********************************************** */
    {
      ...
      n=mySum;
        if(n<=1) {
        //WEIBULL_ERROR_HANDLER(-2,"Insufficient distinct data in weibull_fit\n");
            ret = -2;
            goto freememory;
        }
      /*  ********************************************** */
    ...
    /* too much uncensored data means a plateau with no max */ 
          if(nuncensored<=0) {
          //WEIBULL_ERROR_HANDLER(-2,"Insufficient distinct data, hit a plateau in weibull_fit\n");
              ret = -2;
              goto freememory;
          }
#endif
...
#else
        if(range<=0) {
        //WEIBULL_ERROR_HANDLER(-2,"Insufficient distinct data range in weibull_fit\n");
            ret = -2;
            goto freememory;
        }
#endif
          ...
          while ((tempVal=weibull_scale_likelihood(upper,x0,frequency,meanUncensored,size)) < 0)
            {
              ...
              assert(upper <= FULL_PRECISION_MAX);
                ...
                //WEIBULL_ERROR_HANDLER(-3,"MLE in wbfit Failed to converge leading for overflow in root finding\n");
                    ret = -3;
                    goto freememory;
                }
#endif
...
#else
          if(code != 1) {
              //WEIBULL_ERROR_HANDLER(-4, "weibull_fit failed, could not find solution in MLE. Probably has insufficnt data distribution (e.g. all same value)...\n");
              ret = -4;
              goto freememory;
          }
#endif
...
//-----------    

    {
          ...
          if(rval<0) {
              //WEIBULL_ERROR_HANDLER(-5,"Failed to fine final parameters settings MLE failed. Memory leaked"); 
              ret = -5;
              goto freememory;
          }
          /* ****************************************** */
          ...
          if(rval<0) {
              //WEIBULL_ERROR_HANDLER(-7,"Cannot compute confidence interval since wnorminv fails. Memory leaked");
              ret = -7;
              goto freememory;
          }
