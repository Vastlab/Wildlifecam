...
double MetaRecognition::CDF(double x)
{
  ...
  return weibull_cdf(translated_x, parmhat[0], parmhat[1]);
};
...
{
  if(inputDataSize < fitting_size-1) {
    fprintf(stderr,"Inputsize less than tail size, cannot fit no model built");
    Reset();
    return -1;
  } 

  if(inputDataSize < fitting_size/2) {
    if(been_warned==0) 
      fprintf(stderr,"Warning, input size less than half the tail size;  model may be meaningless");
    been_warned++;
  } 
  ...
  //small_score = dataPtr[fitting_size-1];  
    ...
    // ymlui
    small_score = inputDataCopy[fitting_size-1];
    if(scores_to_drop>0){
        dataPtr=inputDataCopy;
        fitting_size -= scores_to_drop;
    } else {
        dataPtr=inputDataCopy;
    }
...
int MetaRecognition::FitLow(double* inputData, int inputDataSize)
{
        if(inputDataSize < fitting_size-1) {
          fprintf(stderr,"Inputsize less than tail size, cannot fit no model built");
          Reset();
...
int MetaRecognition::FitHigh(double* inputData, int inputDataSize)
{
        if(inputDataSize < fitting_size-1) {
          fprintf(stderr,"Inputsize less than tail size, cannot fit no model built");
          Reset();
...
int MetaRecognition::FitSVM(svm_node_libsvm* SVMdata, int inputDataSize, int label_of_interest, bool label_has_positive_score, bool reject_complement)
{
  Reset();
  ...
  if(inputDataSize < fitting_size/2) {
    if(been_warned==0) 
      fprintf(stderr,"Warning, input size is less than half the tail;  model may be meaningless");
    been_warned++;
  } 
  ...
  int icnt=0;
  if(reject_complement) {  // default we fit on the complement class and then do rejection to determine probability
    for(int i=0; i < inputDataSize; i++) {
  ...
  if (verbose && sign_of_label_of_interest * sign_of_expected_score <= 0){
    printf("In MetaRecognition::FitSVM,  warning: possible inconsistency average of data to fit has score %d, but expected core is %d\n",
  ...
  if((!label_has_positive_score  && reject_complement)
     || (label_has_positive_score  && !reject_complement)) {
    sign = -1;
...
void MetaRecognition::Save(FILE *outputFile) const
{
	if((outputFile != NULL) && !feof(outputFile))
	{
          fprintf(outputFile, 
                        ...
                        "%21.18g %21.18g  "
                        "%d %f %d "  //sign, alpha, fitting size
                        "%d %21.18g %d\n", //translate,  small_score, scores_to_drop
                        parmhat[0], parmhat[1],
                        parmci[0],parmci[1],
                        parmci[2],parmci[3],
                        sign, alpha, fitting_size,
...
void MetaRecognition::Load(FILE *inputFile)
{
	...
	if((inputFile != NULL) && !feof(inputFile))
	{
          ...
          retcode = fscanf(inputFile, 
                           ...
                           "%lf %lf " //parmci 
                           "%lf %lf "
                           "%d %lf %d "  //sign, alpha, fitting size
...
int MetaRecognition::been_warned=0;
